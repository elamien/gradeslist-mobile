...
export function mapGradescopeAssignmentToUniversal(
  ...
  return {
    ...
    due_date: gradescopeAssignment.due_date?.toISO() || null,        // DateTime -> string
  ...
  switch (gradescopeStatus.toLowerCase()) {
    ...
    default:
      if (gradescopeStatus.startsWith('Submitted (') || gradescopeStatus.startsWith('Graded (')) {
        return gradescopeStatus.startsWith('Graded (') ? UniversalAssignmentStatus.GRADED : UniversalAssignmentStatus.SUBMITTED;
      }
      return UniversalAssignmentStatus.MISSING;
  }
}
